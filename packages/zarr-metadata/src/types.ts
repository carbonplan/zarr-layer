...
export interface CRSInfo {
  ...
  code: string | null
  /** Proj4 definition string (or WKT from crs_wkt) for custom CRS, or null */
  proj4def: string | null
  /** Source of CRS information */
  source: 'explicit' | 'grid_mapping' | 'inferred' | 'default'
  /**
   * Scale factor for coordinate array values.
   * For geostationary projections, coordinates may be in scanning angle radians
   * rather than meters — multiply by this value (perspective_point_height) to convert.
   * null if no scaling is needed.
   */
...
export interface CFGridMappingAttributes {
  ...
  scale_factor_at_projection_origin?: number
  false_easting?: number
  false_northing?: number
  standard_parallel?: number | number[]
  /** Geostationary satellite height in meters */
  perspective_point_height?: number
  /** Geostationary sweep angle axis: 'x' or 'y' */
  sweep_angle_axis?: string
  /** Polar stereographic: longitude down from pole */
  straight_vertical_longitude_from_pole?: number
  /** Rotated lat-lon: grid north pole latitude */
  grid_north_pole_latitude?: number
  /** Rotated lat-lon: grid north pole longitude */
  grid_north_pole_longitude?: number
  /** Rotated lat-lon: north pole grid longitude */
  north_pole_grid_longitude?: number
  /** Oblique mercator: azimuth of central line */
  azimuth_of_central_line?: number
  [key: string]: unknown
}
